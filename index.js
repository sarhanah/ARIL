...
const vcard = 'BEGIN:VCARD\n' // JAN DI UBAH
            + 'VERSION:3.0\n' // JAN DI UBAH
            + 'FN: David Gans:v\n' // NAMA KONTAK OWNER
            + 'ORG:Creator MIKU BOT;\n' // NAMA CREATOR
            + 'TEL;type=CELL;type=VOICE;waid=628882379922:+62 888-2379-922\n' // NOMER HP LU
            + 'END:VCARD'
prefix = '#'
...
async function starts() {
	...
	client.on('qr', () => {
		console.log(color('[','white'), color('!','red'), color(']','white'), color(' Scan QR SUBSCRIBE MIKU OFFICIAL BOT!'))
	})
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Bye Kalau Mau Balik Lagi Nitip Gorengan ya:v @${num.split('@')[0]}👋`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '_Tunggu Bentar... Gak bakal lama kek dia:v_',
				asik: '_Tunggu Bentar... Gak bakal lama kek dia:v_',
				success: 'Berhasil ✔️',
				error: {
				...
				only: {
					...
					mod: 'PERINTAH INI KHUSUS USER *MikuBOT*',
					benned: 'Anda Ke Band Silahkan Hubungi Owner Agar Membuka Band Anda',
					ownerG: '_Fitur Ini Hanya Bisa Di Gunakan Oleh Owner Hatsune Miku!_',
					ownerB: '_Fitur Ini Hanya Bisa Di Gunakan Oleh Owner Hatsune Miku!_',
					userB: `──「 DAFTAR 」──\nHalo gan !\nKamu belum Terdaftar didalam database, \n\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar David|17\n\n──「 HATSUNE MIKU BOT 」──`,
					admin: '[❗] Perintah ini hanya bisa di gunakan oleh admin Hatsune Miku!',
					Badmin: ' [❗] Perintah ini hanya bisa di gunakan ketika bot menjadi admin! ',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["628882379922@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"628882379922@s.whatsapp.net"]
			const adminbotnumber = ["628882379922@s.whatsapp.net"]
			const frendsowner = ["628882279922@s.whatsapp.net"]
			const premium = ["628882379922@s.whatsapp.net"]
			...
			switch(command) {
				...
				if (!isPublic) return reply(mess.only.public)
					...
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*MIKU GROUP*\n\nLink : https://chat.whatsapp.com/KlIZyLWPIsx378KMAYdcPo*', text, { quoted: mek })
					break
					case 'funmenu':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					if (mentioned.length > 1) {
						...
						mentions(`╭────「 *PREMIUM👑* 」──*\n│+ *Number* : @${mentioned[0].split('@')[0]}\n│+ *Expired*: *30 Days*\n│+ *Status*: *ACTIVE*\n│ Thx for Upgrade to Premium🥰\n*╰──────「 *HATSUNE MIKU BOT* 」────`, mentioned, true)
					client.sendMessage(from, mentioned)
				    }
					break
					case 'dellvip':
...
if (isBanned) return reply(mess.only.benned)    
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					client.sendMessage(from, '*Gopay :* _0888-2379-922_\n*Saweria :* _https://saweria.co/hatsunemiku21\n*Dana :* _0888-2379-922_', text, { quoted: mek })
					break
                case 'ttp':
                if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('628882279922@s.whatsapp.net', options, text, {quoted: mek})
				...
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('628882379922@s.whatsapp.net', options, text, {quoted: mek})
